...
def GetOrderedPresentsStartingAtTop(solution, submission):
    ...
    presents = {}
    orderedPresents = {}
    for presentId in list(submission.keys()):
...
if __name__ == "__main__":
    ...
    path = './'
    presentsFilename = os.path.join(path, 'dataset/presents.csv')
    submissionFilename = os.path.join(path, 'solution.out')
    ...
    for zheight in sorted(iter(orderedPresents.keys()), reverse=True):
